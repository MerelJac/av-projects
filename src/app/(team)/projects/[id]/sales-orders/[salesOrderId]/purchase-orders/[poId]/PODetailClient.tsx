...
> = {
  DRAFT: { label: "Draft", color: "bg-gray-100 text-gray-600", icon: <Clock size={13} /> },
  SENT: { label: "Sent to Vendor", color: "bg-blue-100 text-blue-700", icon: <Send size={13} /> },
  PARTIALLY_RECEIVED: { label: "Partially Received", color: "bg-amber-100 text-amber-700", icon: <AlertCircle size={13} /> },
  RECEIVED: { label: "Fully Received", color: "bg-green-100 text-green-700", icon: <CheckCircle2 size={13} /> },
};
  ...
  return (
    <div className="min-h-screen bg-[#F7F6F3]">
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        {/* Back */}
        <button
          onClick={() => router.push(`/projects/${projectId}/sales-orders/${salesOrderId}`)}
        ...
        <div className="flex items-start justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-[#666] mt-1 font-semibold">{po.vendor}</p>
          </div>
          <div className="flex items-center gap-3">
            {po.status === "DRAFT" && (
              <button
        ...
        <div className="grid grid-cols-3 gap-6">
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-[#F0EEE9]">
                    ...
                    <th className="text-right text-[10px] font-semibold uppercase tracking-widest text-[#999] px-3 py-3 w-24">Ordered</th>
                    <th className="text-right text-[10px] font-semibold uppercase tracking-widest text-[#999] px-3 py-3 w-24">Received</th>
                    <th className="text-right text-[10px] font-semibold uppercase tracking-widest text-[#999] px-5 py-3 w-24">Cost</th>
                  </tr>
                </thead>
                <tbody>
                  {po.lines.map((line) => {
                    const pct = line.quantity > 0 ? (line.receivedQuantity / line.quantity) * 100 : 0;
                    const done = pct >= 100;
                    return (
                      <tr key={line.id} className="border-b border-[#F7F6F3] last:border-0">
                        <td className="px-5 py-3">
                          <p className="text-sm font-mono text-[#111]">{line.item.itemNumber}</p>
                          {line.item.manufacturer && (
                            <p className="text-[10px] text-[#999]">{line.item.manufacturer}</p>
                          )}
                          {line.salesOrderLine && (
                        ...
                        <td className="px-3 py-3 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <span className={`text-sm font-semibold ${done ? "text-green-600" : line.receivedQuantity > 0 ? "text-amber-600" : "text-[#999]"}`}>
            ...
            <div className="bg-white border border-[#E5E3DE] rounded-2xl overflow-hidden">
              <div className="px-5 py-3.5 border-b border-[#F0EEE9] flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-xs font-semibold uppercase tracking-widest text-[#888]">Shipments</p>
                  {po.shipments.length > 0 && (
                    <span className="text-xs text-[#bbb]">{po.shipments.length}</span>
                  )}
                </div>
                {(po.status === "SENT" || po.status === "PARTIALLY_RECEIVED") && (
                  <button
                    ...
                    <div key={s.id} className="px-5 py-4">
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-start gap-3">
                          <Truck size={14} className="text-[#999] mt-0.5 flex-shrink-0" />
                          <div>
                            <p className="text-sm font-semibold text-[#111]">
                              {s.item?.itemNumber ?? "General shipment"} × {s.quantity}
                            </p>
                            <div className="flex items-center gap-3 mt-1 flex-wrap">
                              {s.carrier && (
                                <span className="text-xs text-[#666]">{s.carrier}</span>
                              )}
                              {s.tracking && (
                              ...
                              {s.shippedBy && (
                                <span className="text-xs text-[#999]">from {s.shippedBy}</span>
                              )}
                            </div>
                            {s.notes && (
                              <p className="text-xs text-[#999] mt-1">{s.notes}</p>
                            )}
                          </div>
                        </div>
                        <div className="text-right flex-shrink-0">
                          {s.receivedAt ? (
                            <span className="text-xs font-semibold text-green-600 flex items-center gap-1">
                              ...
                              Received {s.receivedAt.toLocaleDateString()}
                            </span>
                          ) : (
                            <span className="text-xs text-[#999]">In transit</span>
                          )}
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white border border-[#E5E3DE] rounded-2xl p-5">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-semibold text-[#111]">{po.lines.length}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#666]">Units ordered</span>
                  <span className="font-semibold text-[#111]">{totalOrdered}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-[#666]">Units received</span>
                  <span className={`font-semibold ${totalReceived >= totalOrdered ? "text-green-600" : totalReceived > 0 ? "text-amber-600" : "text-[#999]"}`}>
            ...
            {/* Details */}
            <div className="bg-white border border-[#E5E3DE] rounded-2xl p-5">
              <p className="text-xs font-semibold uppercase tracking-widest text-[#888] mb-4">Details</p>
              <div className="space-y-2 text-sm">
                <div>
                  <p className="text-[10px] text-[#999] uppercase tracking-widest">Vendor</p>
                  <p className="text-[#111] font-semibold mt-0.5">{po.vendor}</p>
                </div>
                <div>
                  <p className="text-[10px] text-[#999] uppercase tracking-widest">Created</p>
                  <p className="text-[#666] mt-0.5">{po.createdAt.toLocaleDateString()}</p>
                </div>
                {po.notes && (
                  <div>
                    <p className="text-[10px] text-[#999] uppercase tracking-widest">Notes</p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
        <div className="px-6 py-4 border-b border-[#F0EEE9] flex items-center justify-between">
          ...
          <button onClick={onCancel} className="text-[#999] hover:text-[#111]"><X size={16} /></button>
        </div>
        <div className="px-6 py-4 space-y-3">
            ...
            >
              ...
              {po.lines.map((l) => (
                <option key={l.itemId} value={l.itemId}>
                  {l.item.itemNumber} — {l.receivedQuantity}/{l.quantity} received
                </option>
              ))}
            </select>
          </div>
          ...
          <div>
            ...
            <input type="text" value={shippedBy} onChange={(e) => setShippedBy(e.target.value)}
        ...
        <div className="px-6 py-4 border-t border-[#F0EEE9] flex justify-end gap-3">
          <button onClick={onCancel} className="text-sm text-[#666] hover:text-[#111]">Cancel</button>
