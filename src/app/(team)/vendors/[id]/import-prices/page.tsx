...
const PRICE_FIELDS = [
  { key: "itemNumber", label: "Item Number", required: true, hint: "Must match an existing item" },
...
function autoMap(headers: string[]): Record<string, string> {
  ...
  const aliases: Record<string, string[]> = {
    ...
    cost:         ["cost", "unitcost", "purchaseprice", "buyprice", "price"],
    leadTimeDays: ["leadtimedays", "leadtime", "lead", "days"],
...
type ImportResult = {
  ...
  updated: number;
  skipped: number;
  errors: { row: number; itemNumber: string; reason: string }[];
};

export default function ImportVendorPricesPage() {
  ...
  const mappedRows = rows.map(row =>
    Object.fromEntries(
      Object.entries(mapping).filter(([, col]) => col).map(([field, col]) => [field, row[col] ?? ""])
    )
  );

  const canImport = PRICE_FIELDS.filter(f => f.required).every(f => mapping[f.key]);

  async function handleImport() {
    setLoading(true);
    try {
      const csv = [
        Object.keys(mapping).filter(k => mapping[k]).join(","),
        ...rows.map(row =>
          Object.keys(mapping).filter(k => mapping[k]).map(k => row[mapping[k]] ?? "").join(",")
        ),
      ].join("\n");
      ...
      const res = await fetch(`/api/vendors/${vendorId}/price-list`, {
        method: "POST",
        body: formData,
      });
    ...
    <div className="bg-[#F7F6F3] min-h-screen">
      <div className="max-w-4xl mx-auto px-6 py-10">
        <div className="flex items-center gap-3 mb-8">
          <Link href={`/vendors/${vendorId}`} className="flex items-center gap-1.5 text-sm text-[#666] hover:text-[#111] transition-colors">
            <ArrowLeft size={14} /> Vendor
          </Link>
          <span className="text-[#ccc]">/</span>
          <span className="text-sm text-[#111] font-semibold">Import Price List</span>
        </div>

        <div className="mb-6">
          <h1 className="text-2xl font-bold text-[#111] tracking-tight">Import Vendor Price List</h1>
          <p className="text-sm text-[#888] mt-1">
            Upload a CSV to bulk create or update vendor item prices. Items are matched by Item Number.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 mb-8">
          {["upload", "map", "preview", "done"].map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              ...
              <span className={`text-xs font-medium capitalize ${step === s ? "text-[#111]" : "text-[#aaa]"}`}>{s}</span>
        ...
        {step === "upload" && (
          <>
            ...
            >
              ...
              <p className="text-xs text-[#aaa]">Required columns: itemNumber, cost</p>
            </div>
            <div className="mt-6 flex items-center gap-2 text-xs text-[#aaa]">
              ...
              <code className="bg-white border border-[#E5E3DE] px-2 py-0.5 rounded text-[#666] font-mono">
                itemNumber, cost, leadTimeDays, notes
              </code>
            </div>
          </>
        )}
              ...
              <div className="p-5 space-y-3">
                ...
                {PRICE_FIELDS.map(field => (
                  <div key={field.key} className="flex items-center gap-4">
                    ...
                    {mapping[field.key] ? <CheckCircle2 size={14} className="text-green-500 shrink-0" /> : <div className="w-3.5 shrink-0" />}
                  </div>
                ))}
              </div>
            </div>
            <div className="flex justify-between">
              <button onClick={reset} className="text-sm text-[#888] hover:text-[#111] transition-colors">← Start over</button>
        ...
        {step === "preview" && (
          <div className="space-y-4">
                  ...
                  <thead>
                    <tr className="border-b border-[#F0EEE9]">
                      {Object.keys(mappedRows[0] ?? {}).map(key => (
                        <th key={key} className="text-left text-[10px] font-semibold uppercase tracking-widest text-[#999] px-4 py-3">{key}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div className="flex justify-between">
              <button onClick={() => setStep("map")} className="text-sm text-[#888] hover:text-[#111] transition-colors">← Back to mapping</button>
        ...
        {step === "done" && result && (
          <div className="space-y-4">
                  ...
                  <div className="divide-y divide-red-50 max-h-48 overflow-y-auto">
                    {result.errors.map((e, i) => (
                      <div key={i} className="flex items-start gap-3 px-4 py-2.5">
                        ...
                        <span className="text-xs text-[#aaa] font-mono shrink-0">{e.itemNumber}</span>
            ...
            <div className="flex justify-between">
              <button onClick={reset} className="text-sm text-[#888] hover:text-[#111] transition-colors">Import another file</button>
              <Link href={`/vendors/${vendorId}`} className="text-sm font-semibold bg-[#111] text-white px-5 py-2 rounded-xl hover:bg-[#333] transition-colors">
                View Vendor →
              </Link>
            </div>
          </div>
